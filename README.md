...
символов через SCIP protobuf) → джойн по спанам → узлы/рёбра, у каждого из
которых явно помечены `resolution` (`static` / `dynamic` / `heuristic`) и
...
по stale-набору внутри изменённого. На фикстурном перф-гейте
(`tests/eval/test_incremental_gate.py`) правка одного файла даёт
`--incremental` время ≈30–40% от полного холодного прогона той же копии — не
«в разы быстрее», честный частичный выигрыш.
...
- **`scip-python` не файл-инкрементален** (см. «Инкрементальный индекс» выше) —
  внутри изменённого сервиса SCIP перегоняется целиком; `--incremental` даёт
  честные ≈30–40% времени полного прогона на правке одного файла, а не
  «мгновенно».
