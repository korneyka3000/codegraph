...
- **Для русскоязычных команд: рассмотрите `intfloat/multilingual-e5-base`
  вместо дефолтного `jina-embeddings-v2-base-code`.** Эксперимент
  ([отчёт](docs/superpowers/reports/2026-07-18-m5-ru-retrieval-experiment.md),
  8 golden-вопросов, детерминированный `--exact` режим, 2 идентичных прогона на
  модель) показал +2 вопроса на hit@8 (5/8 → 7/8) и +1 на hit@3 (2/8 → 3/8) —
  выигрыш сосредоточен на чисто-русских концептуальных вопросах без буквального
  ID-пересечения, где код-специализированная jina слабее настоящей multilingual-
  модели. Не универсальный выигрыш (один вопрос из 8 просел) и не смена
  дефолта — opt-in для команд с преимущественно русскоязычными запросами, через
  `codegraph.yaml`:
  ```yaml
  embedding:
    provider: local
  ...
  ```
  (`query_prefix`/`passage_prefix` — канонические e5-инструкции, отличающие
  поисковый запрос от индексируемого пассажа; пустая строка по умолчанию для
  любой другой модели, включая дефолтную jina.)
